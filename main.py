...
async def pdf_interpret(user: FileInformation):
    ...
    file_collate_xuanhao, file_collate_create = await asyncio.gather(
        asyncio.to_thread(llm, pdf_content, pr.prompt_xuanhao),
        asyncio.to_thread(llm, pdf_content, pr.prompt_create)
    )
    ...
    result = {
        "选号阶段": {
            "产品基础信息": file_collate_xuanhao["产品基础信息"],
            "合作情况": file_collate_xuanhao["合作情况"],
            "其他要求": file_collate_xuanhao["其他要求"],
            "竞品信息": file_collate_xuanhao["竞品信息"],
        },
        "内容创作": {
            "产品基础信息": file_collate_create["产品基础信息"],
            "卖点信息": file_collate_create["卖点信息"],
            "创作要求": file_collate_create["创作要求"],
            "创作注意事项": file_collate_create["创作注意事项"],
            "其他信息": file_collate_create["其他信息"],
        },
    }
